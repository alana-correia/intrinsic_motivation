...
- normalizar a reward
- introducao do contexto como input + introducao do modulo de pred_action
'''
...
class AgentCuriosity(nn.Module):
    def __init__(self, framestack, ninp=512, nhid=[512], nlayers=1, dropout=0.5, num_blocks=[4], topk=[2],
                 use_inactive=False, blocked_grad=False):
        ...
        self.encoder_action = nn.Sequential(
            layer_init(nn.Linear(1, ninp//2)),
            nn.ReLU())
        self.fc3_encoder = layer_init(nn.Linear(ninp//2, ninp))
        self.actor = layer_init(nn.Linear(self.nhid[-1], 4), std=0.01)
        self.critic = layer_init(nn.Linear(self.nhid[-1], 1), std=1)
        self.actor_f = layer_init(nn.Linear(self.nhid[-1], 4), std=0.01)
    ...
    def get_action_and_value(self, x, lstm_state, action=None):
        ...
        if action is None:
            action = probs.sample()
        return action.detach(), probs.log_prob(action), probs.entropy(), self.critic(hidden), lstm_state, embs, hidden

    def compute_intrinsic_reward(self, enc_ts, actions, actual_brims_p, exp_hidden_f, lstm_state):
        batch_size = lstm_state[0][0].shape[0]
        input_size = lstm_state[0][0].shape[1]
        enc_ts = enc_ts.reshape((-1, batch_size, input_size))

        actions = actions.to(device)
        actions = actions.reshape((-1, batch_size, 1))
        ...
        for enc_t, action_t in zip(enc_ts, actions):
            #print(f'emb (dentro do for): {enc_t.shape}')
            #print(f'emb (dentro do for): {enc_ts.shape}')
            #print(f'action (dentro do for): {action_t.shape}')
            action_f1 = self.encoder_action(action_t)
            what_action = self.fc3_encoder(action_f1)

            g_t = F.relu(enc_t + what_action)
            #exit()
            lstm_state = self.brims_f(g_t, lstm_state)
            # new_hidden += [lstm_state[0][-1]]
        ...
        # print(f'após a chamada da lstm new hidden shape: {new_hidden.shape}')
        # exit()
        pred_action = self.actor_f(new_hidden_f)
        eta = 0.01
        intrinsic_reward = eta * F.mse_loss(actual_brims_p, exp_hidden_f, reduction='none').mean(-1)
        return intrinsic_reward.detach().data.cpu().numpy(), lstm_state, new_hidden_f, pred_action.detach().data.cpu().numpy()
...
if __name__ == "__main__":
    ...
    if args.norm_obs:
        ...
        next_obs_norm = np.reshape(next_obs_norm, (
        args.obs_norm_step, next_obs_norm.shape[2], next_obs_norm.shape[3], next_obs_norm.shape[4]))
    ...
    for update in range(update_init, num_updates + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                ...
                action, logprob, _, value, next_lstm_state_p, embs_t, actual_hidden = agent.get_action_and_value(((
                                                                                                                          ...
                                                                                                                          obs_rms.mean).to(
                                                                                                                          device)) / torch.Tensor(
                    np.sqrt(obs_rms.var)).to(device)), next_lstm_state_p)
                # hidden = repackage_hidden(hidden)


                im_reward, next_lstm_state_f, exp_hidden = agent.compute_intrinsic_reward(embs_t, action.type(torch.FloatTensor), actual_hidden,
            ...
            for id, item in enumerate(info):
                if "episode" in item.keys():
                    print(
                        f"update={update}/total_updates={num_updates}, global_step={global_step}, episodic_return={item['episode']['r']}")
        ...
        with torch.no_grad():
            ...
            next_value = agent.get_value(
                ((next_obs - torch.Tensor(obs_rms.mean).to(device)) / torch.Tensor(np.sqrt(obs_rms.var)).to(device)),
                next_lstm_state_p).reshape(1, -1)
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.num_envs, envsperbatch):
                ...
                _, newlogprob, entropy, newvalue, _, embs, h_current = agent.get_action_and_value(
                    ((b_obs[mb_inds] - torch.Tensor(obs_rms.mean).to(device)) / torch.Tensor(np.sqrt(obs_rms.var)).to(
                        device)),
                    (hx_mb_p, cx_mb_p),
                ...
                _, _, exp_hidden = agent.compute_intrinsic_reward(embs_t, b_actions[mb_inds], actual_hidden, exp_hidden, (hx_mb_f, cx_mb_f))
