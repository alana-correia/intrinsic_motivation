    ...
    for idx in range(num_games):
        ...
        while not done:
            with torch.no_grad():
                ...
                next_obs = np.clip(next_obs, a_min=0)
                done = done[-1]
                r += reward[-1]
                print('\rgame {} - reward (sum) {} - done {} - action {}'.format(idx, r, done, action.cpu().numpy()), end='')
                next_obs = torch.Tensor(next_obs).to(device)
...
def main():
    ...
    stats_II_v1 = test_II(args, agent, run_name, path_load, os.path.join(path_save, "test_II_v1"), "test_II_v1", num_games, num_envs,
                     device, 10)

    stats_II_v2 = test_II(args, agent, run_name, path_load, os.path.join(path_save, "test_II_v2"), "test_II_v2", num_games,
                       num_envs,
                       device, 30)

    stats_II_v3 = test_II(args, agent, run_name, path_load, os.path.join(path_save, "test_II_v3"), "test_II_v3", num_games,
                       num_envs,
                       device, 50)

    stats_II_v4 = test_II(args, agent, run_name, path_load, os.path.join(path_save, "test_II_v4"), "test_II_v4", num_games,
                       num_envs,
                       device, 80)
    ...
    stats_VI = test_VI(args, agent, run_name, path_load, os.path.join(path_save, "test_VI"), "test_VI", 5, num_games,
                       num_envs,
                       device)

    dict_data = {'test_I': stats_I, 'test_II_v1': stats_II_v1, 'test_II_v2': stats_II_v2,'test_II_v3': stats_II_v3,'test_II_v4': stats_II_v4, 'test_III': stats_III, 'test_IV': stats_IV, 'test_V': stats_V, 'test_VI': stats_VI}
